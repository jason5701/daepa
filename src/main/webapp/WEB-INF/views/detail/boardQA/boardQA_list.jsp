	...
	<div>
		<table>
			...
			<tr >  <!-- onClick="location.href='boardQA_read?boardQA_number=${vo.boardQA_number}'" -->
				<td style="text-align:center;" id="td_copy(tdVal.value)" >${vo.boardQA_number}</td>
				<td ><a href="#" id="viewhidden${vo.boardQA_number}" onclick="return false;" class="button">${vo.boardQA_title}</td>
			...
			<td colspan = "4">숨김메뉴</th> <td><input type="text"/></td> 
			</tr>
...
<script type="text/javascript"> $(document).ready(function () { 
	...
	$("#viewhidden2").click(function () { 
		...
		if (status == "none") { $("#hidden2").css("display", ""); } 
		else { $("#hidden2").css("display", "none"); } 
		}); 
	...
	$("#viewhidden1").click(function () { 
		...
		status = $("#hidden1").css("display"); 
		if (status == "none") { $("#hidden1").css("display", ""); } 
		else { $("#hidden1").css("display", "none"); } 
		}); 
	}); 
